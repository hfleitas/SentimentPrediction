...
class SqlInstance
{
    ...
    SqlInstance($isShared, $instanceName, $sqlVersion)
    {
        ...
        if ($this.IsShared)
        {
            foreach ($language in [SqlInstance]::Languages.Values)
            {
                ...
                if ($sharedKey -ne $null)
                {
                    ...
                    $this.RSetupPath = "$($this.RootPath)\Setup Bootstrap\SQL2019CTP3.0\x64\RSetup.exe" -f (ConvertSqlVersionToName $sqlVersion)
                    break
                }
            }
        }
        else
        {
            ...
            $this.RSetupPath = "{0}\SQL2019CTP3.0\x64\RSetup.exe" -f`
        ...
        elseif ($this.MlsVersion.ToString(3) -eq "9.2.1"`
			...
			-or $this.MLsVersion.ToString(3) -eq "9.4.7")
        {
