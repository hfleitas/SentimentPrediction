...
class SqlInstance
{
    ...
    SqlInstance($isShared, $instanceName, $sqlVersion)
    {
        ...
        if ($this.IsShared)
        {
            foreach ($language in [SqlInstance]::Languages.Values)
            {
                ...
                if ($sharedKey -ne $null)
                {
                    ...
                    $this.RSetupPath = "$($this.RootPath)\Setup Bootstrap\SQL2019RC1\x64\RSetup.exe" -f (ConvertSqlVersionToName $sqlVersion)
                    break
                }
            }
        }
        else
        {
            ...
            $this.RSetupPath = "{0}\SQL2019RC1\x64\RSetup.exe" -f`
